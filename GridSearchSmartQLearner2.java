...
public class GridSearchSmartQLearner2 {
    ...
    private static double playOneGame(BlackJackEnv game, double[][][][] QTable, double learningRate, double discountFactor) {
        ...
        for (int i=0; i<1; i++) {
            ...
            while (gamestate.get(0).equals("false")) { // Game is not over yet
                ...
                // Get the value of the first card the dealer is holding
                int dealersCard = BlackJackEnv.dealerFirst(BlackJackEnv.getDealerCards(gamestate));
                
                // Old Q value - value in QTable of current state and action = hit
                double oldQValue = QTable[current_state][0][hasActiveAce][dealersCard]; // Update according to action and active ace
                
                // Find the maximum achievable value when moving from the current state and having the QTable
                double maxNextQValue = getMaxQValue(current_state, hasActiveAce, dealersCard, QTable); // Update according to active ace

                // Now we know max Q and now we need action that led to this max Q
                int maxNextQValueACTION = (int) getMaxQValueACTION(current_state, maxNextQValue, hasActiveAce, dealersCard, QTable); // Update according to action and active ace
    ...
    private static double getMaxQValue(int state, int hasActiveAce, int dealersCard, double[][][][] QTable) {
        double maxQValue = Double.NEGATIVE_INFINITY;
        for (int action = 0; action < 2; action++) {
            if (QTable[state][action][hasActiveAce][dealersCard] > maxQValue) {
                maxQValue = QTable[state][action][hasActiveAce][dealersCard];
            }
        }
        return maxQValue;
    }
    ...
    private static double getMaxQValueACTION(int state, double maxQValue, int hasActiveAce, int dealersCard, double[][][][] QTable) {
        int action = 0;
        ...
        for (int k = 0; k < QTable.length; k++) {
            for (int j = 0; j < QTable[k].length; j++) {
                for(int i = 0; i < QTable[k][j].length; i++){
                    for(int l = 0; l < QTable[k][j][i].length; l++){
                        System.out.print(QTable[k][j][i][l] + " ");
                    }
                }
            }
        }
